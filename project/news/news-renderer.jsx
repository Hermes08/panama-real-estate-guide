const NEWS_ID = window.location.pathname.split('/').pop().replace('.html', '');
    // Body lookup: prefer the actual newsBodies entry for this slug. If missing
    // (some recent /news/<slug>.html shells were generated by the scraper before
    // their body content was authored), synthesize a minimal stub from the news
    // list metadata so we never show the WRONG article content (was falling back
    // to Object.values(...)[0] which is palma-blanca — caused QA-v4 P1).
    ...
    if (!n) {
      if (_newsMeta) {
        ...
        // True 404 — slug not in news list at all. Show neutral message.
