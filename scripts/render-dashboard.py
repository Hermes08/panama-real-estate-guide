...
REPORTS_DIR = ROOT / "output" / "refreshes"
AUDIT_REPORTS_DIR = ROOT / "output" / "audits"
OUT_DIR = ROOT / "output"
...
def find_latest_vital_report() -> Path | None:
    if not REPORTS_DIR.exists():
        return None
    candidates = sorted(REPORTS_DIR.glob("*-refresh.md"), reverse=True)
...
def render_onsite_tab(onsite: dict) -> str:
    ...
    # Find the latest audit markdown report and embed it
    report_path = None
    if AUDIT_REPORTS_DIR.exists():
        candidates = sorted(AUDIT_REPORTS_DIR.glob("*-audit.md"), reverse=True)
...
def render_refresh_tab(candidates: dict, refresh: dict) -> str:
    ...
    if REPORTS_DIR.exists():
        candidates_md = sorted(REPORTS_DIR.glob("*-refresh.md"), reverse=True)
...
def main() -> int:
    ...
    site = bank.get("site") or "panamarealestateguide.com"
